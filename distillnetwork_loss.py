...
#     def __init__(self, origin_model, subnet_model, temperature=1.0, loss_weights=DistillationLossWeights(0.5,0.5)):
...
class loss(nn.Module):
    def __init__(self, origin_model, subnet_model, temperature=1.0, loss_weights=DistillationLossWeights(0.5,0.5,0)):
        super(loss, self).__init__()
        self.origin_model = origin_model
        self.subnet_model = subnet_model
        self.temperature = temperature
    ...
    def forward(self, input, target):
        ...
        if self.loss_weights.origin == 0:
            with torch.no_grad():
                self.last_origin_logits = self.origin_model(input)
        else:
            self.last_origin_logits = self.origin_model(input)

        self.last_subnet_logits = self.subnet_model(input)

        soft_labels = F.log_softmax(self.last_origin_logits / self.temperature, dim=1)
        ...
        self.loss_distill = F.kl_div(soft_preds, soft_labels)
